...
| 姓名 | 描述 |
| --- | --- |
| [Action](../../aspose.diagram/eventitem/action) { get; set; } | 指定父 EventItem 元素的操作代码。对于要保存在 DatadiagramML 文件中的 EventItem 元素，它必须是可持久的。目前，可持久事件可以具有的唯一有效操作代码是 1 (ONEVENT_ACT_RUNADDON)。 |
| [Enabled](../../aspose.diagram/eventitem/enabled) { get; set; } | 表示一个标志，指示事件是启用还是禁用。 |
| [EventCode](../../aspose.diagram/eventitem/eventcode) { get; set; } | 指示触发插件的事件的代码。有关事件代码的详细信息，请参阅 Microsoft Visio 2007 自动化参考中的事件代码。 |
