---
title: ObjTypeValue
second_title: Aspose.Diagram لـ Java API Reference
description: يحدد ما إذا كان يمكن وضع الكائنات أو توجيهها في المخططات عند استخدام Microsoft Visio لترتيب الأشكال على صفحة الرسم.
type: docs
weight: 253
url: /ar/java/com.aspose.diagram/objtypevalue/
---

**Inheritance:**
java.lang.Object
```
public final class ObjTypeValue
```

يحدد ما إذا كان يمكن وضع الكائنات أو توجيهها في المخططات عند استخدام Microsoft Visio لترتيب الأشكال على صفحة الرسم.
## الحقول

| حقل | الوصف |
| --- | --- |
| [DRAWING_CONTEXT](#DRAWING-CONTEXT) | يقرر Visio بناءً على سياق الرسم. |
| [SHAPE_NOT_PLACEABLE_NOT_ROUTABLE](#SHAPE-NOT-PLACEABLE-NOT-ROUTABLE) | الشكل غير قابل للتثبيت، غير قابل للتوجيه. |
| [SHAPE_PLACEABLE](#SHAPE-PLACEABLE) | الشكل قابل للتثبيت. |
| [SHAPE_PLACEABLE_ROUTABLE](#SHAPE-PLACEABLE-ROUTABLE) | المجموعة تحتوي على أشكال قابلة للتثبيت/التوجيه. |
| [SHAPE_ROUTABLE](#SHAPE-ROUTABLE) | الشكل قابل للتوجيه. |
| [UNDEFINED](#UNDEFINED) | غير معرف. |
## الطرق

| طريقة | الوصف |
| --- | --- |
| [equals(Object arg0)](#equals-java.lang.Object-) |  |
| [getClass()](#getClass--) |  |
| [hashCode()](#hashCode--) |  |
| [notify()](#notify--) |  |
| [notifyAll()](#notifyAll--) |  |
| [toString()](#toString--) |  |
| [wait()](#wait--) |  |
| [wait(long arg0)](#wait-long-) |  |
| [wait(long arg0, int arg1)](#wait-long-int-) |  |
### DRAWING_CONTEXT {#DRAWING-CONTEXT}
```
public static final int DRAWING_CONTEXT
```


يقرر Visio بناءً على سياق الرسم.

### SHAPE_NOT_PLACEABLE_NOT_ROUTABLE {#SHAPE-NOT-PLACEABLE-NOT-ROUTABLE}
```
public static final int SHAPE_NOT_PLACEABLE_NOT_ROUTABLE
```


الشكل غير قابل للتثبيت، غير قابل للتوجيه.

### SHAPE_PLACEABLE {#SHAPE-PLACEABLE}
```
public static final int SHAPE_PLACEABLE
```


الشكل قابل للتثبيت.

### SHAPE_PLACEABLE_ROUTABLE {#SHAPE-PLACEABLE-ROUTABLE}
```
public static final int SHAPE_PLACEABLE_ROUTABLE
```


المجموعة تحتوي على أشكال قابلة للتثبيت/التوجيه.

### SHAPE_ROUTABLE {#SHAPE-ROUTABLE}
```
public static final int SHAPE_ROUTABLE
```


الشكل قابل للتوجيه.

### UNDEFINED {#UNDEFINED}
```
public static final int UNDEFINED
```


غير معرف.

### equals(Object arg0) {#equals-java.lang.Object-}
```
public boolean equals(Object arg0)
```




**Parameters:**
| معامل | نوع | الوصف |
| --- | --- | --- |
| arg0 | java.lang.Object |  |

**Returns:**
منطقي
### getClass() {#getClass--}
```
public final native Class<?> getClass()
```




**Returns:**
java.lang.Class<?>
### hashCode() {#hashCode--}
```
public native int hashCode()
```




**Returns:**
int
### notify() {#notify--}
```
public final native void notify()
```




### notifyAll() {#notifyAll--}
```
public final native void notifyAll()
```




### toString() {#toString--}
```
public String toString()
```




**Returns:**
java.lang.String
### wait() {#wait--}
```
public final void wait()
```




### wait(long arg0) {#wait-long-}
```
public final native void wait(long arg0)
```




**Parameters:**
| معامل | نوع | الوصف |
| --- | --- | --- |
| arg0 | long |  |

### wait(long arg0, int arg1) {#wait-long-int-}
```
public final void wait(long arg0, int arg1)
```




**Parameters:**
| معامل | نوع | الوصف |
| --- | --- | --- |
| arg0 | long |  |
| arg1 | int |  |

