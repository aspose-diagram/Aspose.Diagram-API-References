---
title: LineAdjustFromValue
second_title: Aspose.Diagram لـ Java API Reference
description: يحدد أي الموصلات الديناميكية يجب إبعادها إذا كانت تتقاطع فوق بعضها البعض.
type: docs
weight: 223
url: /ar/java/com.aspose.diagram/lineadjustfromvalue/
---

**Inheritance:**
java.lang.Object
```
public final class LineAdjustFromValue
```

يحدد أي الموصلات الديناميكية يجب إبعادها إذا كانت تتقاطع فوق بعضها البعض.
## الحقول

| حقل | الوصف |
| --- | --- |
| [ALL_LINES](#ALL-LINES) | جميع الخطوط. |
| [NO_LINES](#NO-LINES) | لا خطوط. |
| [ROUTING_STYLE_DEFAULT](#ROUTING-STYLE-DEFAULT) | النمط الافتراضي للتوجيه. |
| [UNDEFINED](#UNDEFINED) | غير معرف. |
| [UNRELATED_LINES](#UNRELATED-LINES) | أسطر غير ذات صلة. |
## الطرق

| طريقة | الوصف |
| --- | --- |
| [equals(Object arg0)](#equals-java.lang.Object-) |  |
| [getClass()](#getClass--) |  |
| [hashCode()](#hashCode--) |  |
| [notify()](#notify--) |  |
| [notifyAll()](#notifyAll--) |  |
| [toString()](#toString--) |  |
| [wait()](#wait--) |  |
| [wait(long arg0)](#wait-long-) |  |
| [wait(long arg0, int arg1)](#wait-long-int-) |  |
### ALL_LINES {#ALL-LINES}
```
public static final int ALL_LINES
```


جميع الخطوط.

### NO_LINES {#NO-LINES}
```
public static final int NO_LINES
```


لا خطوط.

### ROUTING_STYLE_DEFAULT {#ROUTING-STYLE-DEFAULT}
```
public static final int ROUTING_STYLE_DEFAULT
```


النمط الافتراضي للتوجيه.

### UNDEFINED {#UNDEFINED}
```
public static final int UNDEFINED
```


غير معرف.

### UNRELATED_LINES {#UNRELATED-LINES}
```
public static final int UNRELATED_LINES
```


أسطر غير ذات صلة.

### equals(Object arg0) {#equals-java.lang.Object-}
```
public boolean equals(Object arg0)
```




**Parameters:**
| معامل | نوع | الوصف |
| --- | --- | --- |
| arg0 | java.lang.Object |  |

**Returns:**
منطقي
### getClass() {#getClass--}
```
public final native Class<?> getClass()
```




**Returns:**
java.lang.Class<?>
### hashCode() {#hashCode--}
```
public native int hashCode()
```




**Returns:**
int
### notify() {#notify--}
```
public final native void notify()
```




### notifyAll() {#notifyAll--}
```
public final native void notifyAll()
```




### toString() {#toString--}
```
public String toString()
```




**Returns:**
java.lang.String
### wait() {#wait--}
```
public final void wait()
```




### wait(long arg0) {#wait-long-}
```
public final native void wait(long arg0)
```




**Parameters:**
| معامل | نوع | الوصف |
| --- | --- | --- |
| arg0 | long |  |

### wait(long arg0, int arg1) {#wait-long-int-}
```
public final void wait(long arg0, int arg1)
```




**Parameters:**
| معامل | نوع | الوصف |
| --- | --- | --- |
| arg0 | long |  |
| arg1 | int |  |

