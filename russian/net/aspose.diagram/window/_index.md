...
description: Представляет открытое окно в экземпляре Microsoft Visio. Этот элемент содержит информацию необходимую для точного воссоздания окна пользовательского интерфейса в рабочей области приложения когда файл DatadiagramML первоначально открывается Visio.
type: docs
weight: 4380
...
Представляет открытое окно в экземпляре Microsoft Visio. Этот элемент содержит информацию, необходимую для точного воссоздания окна пользовательского интерфейса в рабочей области приложения, когда файл DatadiagramML первоначально открывается Visio.
...
| Имя | Описание |
| --- | --- |
| [Container](../../aspose.diagram/window/container) { get; set; } | ID контейнера:Страница, Лист или Мастер. Уместно и необходимо только в том случае, если указан ContainerType. |
| [ContainerType](../../aspose.diagram/window/containertype) { get; set; } | Может быть одним из следующих значений:Документ, Страница или Мастер. Имеет значение только в том случае, если WindowType указан как Drawing или Sheet. |
...
| [GlueSettings](../../aspose.diagram/window/gluesettings) { get; set; } | Определяет объекты, формирующие склеивание, когда склеивание включено в документе. |
...
| [ParentWindow](../../aspose.diagram/window/parentwindow) { get; set; } | ID окна, в котором содержится это окно шаблона. Имеет значение, только если WindowType указан как Stencil. |
| [ReadOnly](../../aspose.diagram/window/readonly) { get; set; } | Флаг только для чтения, если этот шаблон не является шаблоном документа. |
| [Sheet](../../aspose.diagram/window/sheet) { get; set; } | ID листа в контейнере. Имеет значение, только если Контейнер указан как Лист. |
...
| [ShowPageBreaks](../../aspose.diagram/window/showpagebreaks) { get; set; } | Указывает, отображаются ли в окне разрывы страниц. |
...
| [SnapExtensions](../../aspose.diagram/window/snapextensions) { get; set; } | Указывает, включена или отключена конкретная настройка расширения привязки для активного окна. Значение может быть суммой значений в следующей таблице. |
| [SnapSettings](../../aspose.diagram/window/snapsettings) { get; set; } | Задает объекты, к которым привязываются фигуры, когда привязка активна в окне. Значение может быть суммой значений в следующей таблице. |
...
| [StencilGroupPos](../../aspose.diagram/window/stencilgrouppos) { get; set; } | Содержит целое число, указывающее относительное положение шаблона в группе в окне. |
...
| [WindowType](../../aspose.diagram/window/windowtype) { get; set; } | Перечисленное значение, которое может быть одним из следующих:Drawing, Sheet, Stencil или Icon. Элемент Window WindowType='Stencil' должен появиться после его родительского рисунка. window (WindowType='Drawing') и перед любыми другими элементами окна рисования. |
