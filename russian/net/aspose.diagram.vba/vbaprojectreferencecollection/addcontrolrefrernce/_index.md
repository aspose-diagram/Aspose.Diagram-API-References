---
title: AddControlRefrernce
second_title: Справочник по Aspose.Diagram для .NET API
description: Добавляет ссылку на библиотеку перевернутых типов и ее расширенную библиотеку типов.
type: docs
weight: 20
url: /ru/net/aspose.diagram.vba/vbaprojectreferencecollection/addcontrolrefrernce/
---
## VbaProjectReferenceCollection.AddControlRefrernce method

Добавляет ссылку на библиотеку перевернутых типов и ее расширенную библиотеку типов.

```csharp
public int AddControlRefrernce(string name, string libid, string twiddledlibid, 
    string extendedLibid)
```

| Параметр | Тип | Описание |
| --- | --- | --- |
| name | String | Имя ссылки. |
| libid | String | Идентификатор библиотеки типов автоматизации. |
| twiddledlibid | String | Идентификатор библиотеки подкручиваемых типов |
| extendedLibid | String | Идентификатор библиотеки расширенных типов |

### Смотрите также

* class [VbaProjectReferenceCollection](../../vbaprojectreferencecollection)
* пространство имен [Aspose.Diagram.Vba](../../vbaprojectreferencecollection)
* сборка [Aspose.Diagram](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Diagram.dll -->
